...
describe("WASM Module Tests", () => {
	test("parse_and_aggregate_wasm processes sample input", () => {
		...
		const graph = wasmModule.parse_and_aggregate_wasm(content, 5000, null) as any;
	...
	test("WASM with expanded_dirs parameter", () => {
		if (!wasmAvailable || !wasmModule) {
			console.log("Skipping: WASM module not found");
			return;
		}
		...
		const content = readFileSync(sampleCruise, "utf-8");
		const graph = wasmModule.parse_and_aggregate_wasm(content, 5000, ["src"]) as any;
...
describe("Aggregation Tests", () => {
	...
	test("Rust binary: --level directory forces directory aggregation", () => {
		...
		assert.ok(
			graph.nodes.some((n: { node_type: string }) => n.node_type === "directory"),
