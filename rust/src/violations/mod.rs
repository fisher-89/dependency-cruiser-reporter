...
pub(super) fn parse_violations(raw_violations: &[RawViolation]) -> Vec<ViolationInfo> {
    raw_violations
        .iter()
        .filter_map(|v| {
            Some(ViolationInfo {
...
pub(super) fn compute_violation_counts(violations: &[ViolationInfo]) -> HashMap<String, u32> {
...
pub(super) struct EdgeViolationCounts {
    pub error_count: u32,
    pub warn_count: u32,
    pub info_count: u32,
}
...
pub(super) fn build_edge_violations(violations: &[ViolationInfo]) -> HashMap<(String, String), EdgeViolationCounts> {
